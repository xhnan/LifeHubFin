...
const DetailScreen = () => {
  ...
  const fetchDetails = useCallback(async () => {
    ...
    try {
      ...
      const groups = result.dailyGroups || [];
      setDailyGroups(groups);

      let income = 0;
      let expense = 0;
      groups.forEach(group => {
        income += group.dailyIncome;
  ...
  }, [bookId, currentDate, fetchDetails]);
  ...
  const renderTransaction = ({item}: {item: TransactionItem}) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate(ROUTES.transactionDetail, {transaction: item})}
        activeOpacity={0.7}>
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <View style={styles.header}>
        <View style={styles.topRow}>
          ...
          <TouchableOpacity onPress={() => setPickerVisible(true)} style={styles.dateBtn}>
      ...
      <View style={styles.body}>
        {loading ? (
          ...
          <View style={styles.emptyWrap}>
            ...
            <Text style={styles.emptyText}>当前月份暂无账单</Text>
            <Text style={styles.emptyHint}>点击右下角按钮开始记账</Text>
          </View>
        ) : (
          <SectionList
            sections={sections}
              ...
              return (
                <View style={styles.sectionHeader}>
                  <Text style={styles.sectionDate}>
                    {section.date} 周{weekday}
                  </Text>
                  <View style={styles.sectionStats}>
                    {section.dailyIncome > 0 && (
                      <Text style={styles.sectionIncome}>收 {section.dailyIncome.toFixed(2)}</Text>
                    )}
                    {section.dailyExpense > 0 && (
                      <Text style={styles.sectionExpense}>支 {section.dailyExpense.toFixed(2)}</Text>
                    )}
                  </View>
                </View>
              );
        ...
        <View style={styles.fabGroup}>
          <TouchableOpacity
            style={styles.addFab}
            activeOpacity={0.8}
            onPress={() => navigation.navigate(ROUTES.receipt)}
            accessibilityLabel={ROUTES.receipt}>
            <Text style={styles.addFabText}>+</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.voiceFab} activeOpacity={0.8} accessibilityLabel="语音记账">
...
const styles = StyleSheet.create({
  ...
  fabGroup: {position: 'absolute', right: 20, bottom: 28, alignItems: 'center', gap: 12},
  addFab: {
    width: 48,
    height: 48,
    borderRadius: 24,
  ...
  addFabText: {fontSize: 28, color: '#fff', fontWeight: '300', lineHeight: 30},
});
